...
Set @union = @ex+LTRIM(str(@num))
	...
	While @j<16
			...
			--	if @j = 0 or @j = 1 or @j = 2 or @j = 3 or @j = 5 or @j = 10 or @j = 15 
				if @j = 15  -- = m - порог отсечения терминов
				begin
					...
					set @sqlstr1 = ' 
					SELECT dbo.MDT_GLOBAL.Word, CASE WHEN sum_e'+@name+ltrim(str(@i))+' = 0 THEN ((CASE WHEN sum_r'+@name+ltrim(str(@i))+' = 0 THEN 0 ELSE sum_r'+@name+ltrim(str(@i))+' END)) ELSE sum_'+@lang+@name+ltrim(str(@i))+' END AS obuch, CASE WHEN [dbo].[q_'+@exam+'].['+@exam+'] IS NULL 
										  ...
										  ,CASE WHEN [dbo].[q_'+@exam+'].['+@exam+'] IS NULL THEN ''0'' ELSE [dbo].[q_'+@exam+'].['+@exam+'] *  LOG ('+ltrim(str(@obuch))+'.0  / count_docs) END AS examen
					...
					--end  				
					------------------------<
					set @sqlstr1 = 'insert into ccc (M'+ltrim(str(@M))+ '_n , M'+ltrim(str(@M))+ '_d , www)  select  top 10  name, distance, '''+((@Exam))+'''  from BS order by distance desc '
					exec (@sqlstr1)
			...
			--			print('m= ' + str(@m) + ' Exam= ' + str(@t)+ ' p= '+ str(@p) + ' num= ' + str(@num))
							if @p = 1 or @p = 3 or @p = 5 or @p = 7 or @p = 10 exec kbs_check @m =@m ,@Exam = @t, @k= @p, @i= @num, @desc = 'desc'
							set @p = @p+1					
						end
								
				
					select top 10 * from BS GROUP BY Name , distance, keynum order by distance  desc
					exec ('delete from BS')		
					exec('drop table q_'+ @union)     
				end
				set @j = @j + 1
