...
BEGIN
	...
	exec('drop table Union_words_ruseng'+@Name + '_' + @part   )
	exec('drop table MDT_ruseng' + @name  + '_' + @part)
	...
	--Первая строка
	set @sqlstr2 = 'select Word, sum_'+@lang +@name  +  ltrim(str(@num)) + '_' + @part+ ', '
	--rSC
	while(@num <@counttables  ) -- -1, т.к. первый элемент уже записан
		begin 
		set @num  = @num + 1
		set @sqlstr2 = @sqlstr2 + 'sum_' + @lang+@name +  ltrim(str(@num)) + '_' + @part +', '
		--print (@sqlstr2)
	...
	while(@num <@counttables +1)
		...
		set @sqlstr2 = @sqlstr2 + '''0'' as sum_' + @lang+@name +  ltrim(str(@num)) + '_' + @part +', '
		set @num  = @num + 1
		--print (@sqlstr2)
	end

	set @sqlstr2 =  Left(@sqlstr2, len(@sqlstr2) -1 )  + ', count_words, count_docs into Union_words_ruseng'+@Name   + '_' + @part + ' from MDT_r'+@Name   + '_' + @part 
	print(@sqlstr2)
	...
	set @sqlstr2 =  Left(@sqlstr2, len(@sqlstr2) -1 )  + ', count_words, count_docs  from MDT_e'  + @name + '_' + @part
	exec (@sqlstr2)
	print (@sqlstr2)
	...
	while (@i < @counttables +1)
		begin
			set @sqlstr3 = @sqlstr3 + 'SUM(sum_' +@lang+@name +  ltrim(str(@i)) + '_' + @part+ ') AS sum_' +@lang+@name +  ltrim(str(@i)) + '_' + @part+ ', '
			set @sqlstr4 =  @sqlstr4  +'sum_'+@lang+@name +  ltrim(str(@i))  + '_' + @part + ' + '
			set @i = @i + 1
		end
	...
	set @sqlstr3 = @sqlstr3 
	set @sqlstr3 =  Left(@sqlstr3, len(@sqlstr3) -1 )  + ', SUM(count_words) as count_words,  SUM(count_docs) as count_docs, COUNT(*) AS Count_classes  into MDT_ruseng'+@name + '_' + @part+' from Union_words_ruseng'+@Name  + '_' + @part  + ' GROUP BY Word;'
	print (@sqlstr3)	
	exec (@sqlstr3)
	print (@sqlstr3)
