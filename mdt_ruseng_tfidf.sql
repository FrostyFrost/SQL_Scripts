	...
	BEGIN
		...
		while (@i <@counttables +1)
			begin
				...
				exec (@sqlstr1)
				set @sqlstr3 = ('UPDATE mdt_ruseng'+@name +'_tfidf set sum_'+@lang +@name +ltrim(str(@i))+' = sum_'+@lang +@name +ltrim(str(@i))+' * LOG (65.0 / dbo.MDT_ruseng'+@name +'_tfidf.count_docs) ')
				exec (@sqlstr3)
				set @i = @i + 1
			end
		...
		--set @sqlstr2 = 'Update mdt_ruseng'+@name +'_tfidf set count_words = (count_words* LOG (67.0 / dbo.MDT_ruseng'+@name +'_tfidf.count_docs)) '
		--exec (@sqlstr2)
		...
		set	@lang = 'r'	
				...
				while (@i <@counttables +1)
					begin
						...
						exec (@sqlstr1)
						set @sqlstr3 = ('UPDATE mdt_'+@lang+@name +'_tfidf set sum_'+@lang +@name +ltrim(str(@i))+' = sum_'+@lang +@name +ltrim(str(@i))+' * LOG (65.0 / dbo.MDT_'+@lang+@name +'_tfidf.count_docs) ')
						exec (@sqlstr3)
						set @i = @i + 1
					end
