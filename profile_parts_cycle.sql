...
--------Parameters---vvv  
--,@t_ numeric (5,3)	= 1			-- коэффициненты при соответствующих частях
--,@k_ numeric (5,3)	= 1
--,@a_ numeric (5,3)	= 1

,@method_t_  as nvarchar (4) = 'MI_I'	--метод для соответствующих частей ( если set @method_ = rtrim('part') )
,@method_a_  as nvarchar (4) = 'RO'
,@method_k_  as nvarchar (4) = 'MI_I'

set @lang_ = 'a'				-- язык. "а" - смешанный  
set @M_ = 200					-- длина профиля  Центроид: m=5, для профилей - длина профиля (100)
set @method_ = rtrim('part')		-- CHI2, MI, Cent, Ro, MI_I, uni1(lang=a), uni2(lang=a), tfc, S1, UNI3,4 (lang=a), part
...
else if @method_ = 'RO' or  @method_ ='UNI1'
...
if @method_ = 'cent' 
	exec('update centr_results set TM=0, SC=0, NM=0, EO=0, SA=0, CO=0, MC=0, RO=0, DB=0, AU=0')
	...
	exec('update NB_results set TM=0, SC=0, NM=0, EO=0, SA=0, CO=0, MC=0, RO=0, DB=0, AU=0
		...
		while @i < 12 + 1
			begin
				...
				if @method_ = 'CHI2' or  @method_ = 'MI' or @method_ = 'Ro' or  @method_ = 'MI_I'  or  @method_ like 'uni[1234]' or @method_ = 'tfc' or @method_ = 'S1'  or @method_ = 'Part' 
					exec  ('update '+@tblCHI2+' set '+@name_+' = (select top 1 Class from profile_result order by weight desc) where num = ' + @p) --drusengtm для выбора максимального веса. Для выбора минимального - убрать
